...
export default class UserFacade {
    ...
    static async deleteUser(userName: string): Promise<string> { //finish this
    ...
    static async getAllUsers(): Promise<Array<any>> {
        const all = await userCollection.find(
            {},
            {projection: {username: 1, name: 1, _id: 0}} //0 omits the property, 1 returns the property. Password is omitted implicitly.
            )
            return all.toArray();
    }
