...
describe("Verify /gameapi/getPostIfReached", () => {
  ...
  xit("Should find team2 +team3, since both are inside range", async function(){
    const newPosition = {"userName":"t1", "password":"secret", "lat":55.77, "lon": 12.48,"distance": 170 }
    const config = {
        method: 'POST',
        headers: {
          'Accept': 'application/json',
  ...
  xit("Should NOT find team2, since not in range", async function(){
    const newPosition = {"userName":"t1", "password":"secret", "lat":55.77, "lon": 12.48,"distance": 10 }
    const config = {
        method: 'POST',
        headers: {
          'Accept': 'application/json',
  ...
  xit("Should not find team2, since credentials are wrong", async function(){
    try{
    const newPosition = {"userName":"t1", "password":"wrong", "lat":55.77, "lon": 12.48,"distance": DISTANCE_TO_SEARCH }
    const config = {
        method: 'POST',
        headers: {
          'Accept': 'application/json',
    ...
    expect(err.message).to.be.equal("Wrong credentials")
